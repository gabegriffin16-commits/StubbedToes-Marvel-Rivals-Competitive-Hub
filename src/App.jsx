...
// ─── UI (35% larger) ───
function Tab({a,onClick,children,color,m:mob}){const ac=color||C.brand;return <button onClick={onClick} style={{background:a?`${ac}33`:"transparent",color:a?ac:C.dim,border:a?`1px solid ${ac}55`:`1px solid ${C.border}`,padding:mob?"7px 0":"10px 14px",borderRadius:mob?"6px":"8px",cursor:"pointer",fontFamily:"'Rajdhani',sans-serif",fontWeight:700,fontSize:mob?"10px":"14px",letterSpacing:mob?"0.5px":"1px",textTransform:"uppercase",whiteSpace:"nowrap",transition:"all 0.2s ease",textAlign:"center",width:"100%"}}>{children}</button>}
...
<div style={{display:"grid",gridTemplateColumns:m?"repeat(6,1fr)":"repeat(6,1fr)",gap:m?"3px":"6px",alignItems:"center"}}>
...
return <button key={g.label} onClick={()=>setOpenGroup(isOpen?null:g.label)} style={{background:hasActive?`${g.color}22`:isOpen?`${g.color}15`:"transparent",color:hasActive?g.color:isOpen?g.color:C.dim,border:`1px solid ${hasActive||isOpen?`${g.color}44`:C.border}`,padding:m?"7px 0":"10px 14px",borderRadius:m?"6px":"8px",cursor:"pointer",fontFamily:"'Rajdhani',sans-serif",fontWeight:700,fontSize:m?"10px":"14px",letterSpacing:m?"0.5px":"1px",textTransform:"uppercase",whiteSpace:"nowrap",transition:"all 0.2s ease",textAlign:"center",width:"100%"}}>{shortLabel} {isOpen?"▲":"▼"}</button>})}
